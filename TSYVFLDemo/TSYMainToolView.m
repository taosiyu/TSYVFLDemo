...
@interface TSYMainToolView()

@property (nonatomic) NSInteger loadedIndex;//新加载的视图的index
@property (nonatomic) BOOL finishedReset;//滑动结束之后
...
@property (nonatomic) NSMutableArray *currentCards;//所有卡片的数组

@end

@implementation TSYMainToolView
...
- (void)defaultConfig {
    ...
    self.direction = TSYDragDirectionDefault;
    self.loadedIndex = 0;
    self.moving = NO;
}

#pragma mark 初始显示的卡片数量这里是3张卡片
- (void)installNextItem {
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfIndexs)] && [self.dataSource respondsToSelector:@selector(draggableContainer:viewForIndex:)]) {
        ...
        NSInteger indexs = [self.dataSource numberOfIndexs];//卡片总数
        NSInteger preloadViewCont = indexs <= kVisibleCount ? indexs : kVisibleCount;//取前三个
        ...
        if (self.loadedIndex < indexs) {
            ...
            for (long int i = self.currentCards.count; i <  (self.moving ? preloadViewCont + 1: preloadViewCont); i++) {
                ...
                CardView *cardView = [self.dataSource draggableContainer:self viewForIndex:self.loadedIndex];
                ...
                [self sendSubviewToBack:cardView]; // addSubview后添加sendSubviewToBack, 使Card的显示顺序倒置，这样就可以不停的添加但是第一张永远在最前面
...
#pragma mark 重置显示的卡片的view
- (void)resetVisibleCards {
    
    __weak TSYMainToolView *weakself = self;
         ...
         usingSpringWithDamping:0.6
          initialSpringVelocity:0.0
                     ...
                     animations:^{
                         ...
                         weakself.finishedReset = YES;//重置完毕
                     }];
...
- (void)originalLayout {
    ...
    for (int i = 0; i < self.currentCards.count; i++) {
        ...
        CardView *cardView = [self.currentCards objectAtIndex:i];
...
- (void)tapGestureHandle:(UITapGestureRecognizer *)tap {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:didSelectIndex:)]) {
        [self.delegate draggableContainer:self cardView:(CardView *)tap.view didSelectIndex:tap.view.tag];
    }
}

#pragma mark 设置移动状态，移动还是没移动
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:widthRatio:heightRatio:)]) {
            ...
            if (widthRatio > 0) {
                self.direction = TSYDragDirectionRight;
            } if (widthRatio < 0) {
                self.direction = TSYDragDirectionLeft;
...
- (void)finishedPanGesture:(UIView *)cardView direction:(TSYDragDirection)direction scale:(CGFloat)scale disappear:(BOOL)disappear {
    ...
    if (!disappear) {
        ...
        NSInteger flag = direction == TSYDragDirectionLeft ? -1 : 2;
        [UIView animateWithDuration:0.5f
                              delay:0.0
                            options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionAllowUserInteraction
                         animations:^{
                             cardView.center = CGPointMake(TTWidth * flag, TTWidth * flag / scale + self.cardCenter.y);
                         } completion:^(BOOL finished) {
...
- (void)removeFormDirection:(TSYDragDirection)direction {
    ...
    if (self.moving) {
        ...
        switch (direction) {
            case TSYDragDirectionLeft:
                cardCenter = CGPointMake(-TTWidth / 2, self.cardCenter.y);
                flag = -1;
                break;
            case TSYDragDirectionRight:
                cardCenter = CGPointMake(TTWidth * 1.5, self.cardCenter.y);
                flag = 1;
                break;
            default:
                break;
